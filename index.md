# My User Page
Hi! Welcome to my User Page  

## Table of Contents
- [My User Page](#my-user-page)
  - [Table of Contents](#table-of-contents)
    - [Me as a Person](#me-as-a-person)
    - [Me as a Programmer](#me-as-a-programmer)
    - [General Goals](#general-goals)

### Me as a Person  
- I am from San Diego, CA  
- I like reading, speed-cubing, playing bass, and solving puzzles
- I also love cats

![little cat](LittleCat.jpg)
Just look at this little guy
>how adorable
- My Favorite [Video](https://www.youtube.com/watch?v=dQw4w9WgXcQ)
- My most used command in the terminal: `sc delete vgc` and `sc delete vgk`
>because I hate vanguard
- Some of my favorite [songs](songlist.md)

### Me as a Programmer
- My field of interest would fall under **Cybersecurity** or **Cryptograhy**
- Programming languages I know: ***Java***, C, ***C++***, Python
- I have a small project on encoding with simple ciphers
>with more to come ~~hopefully~~
- I also have a group project on what aspects of a movie help determine a films commerical success


### General Goals
- [ ] Learn French
- [ ] Get better at github
- [ ] Start 1-2 projects over the summer
- [ ] Get a sub-10 solve with a Rubik's cube
- [ ] Learn Javascript/CSS
- [ ] Learn a new song on the bass
- [x] Be Awesome
